...
export async function generateMetadata({ params }: PageProps) {
  ...
  const procedureData = getProcedureBySlug("cirugia-puerto-unico", locale as Locale);
  if (!procedureData) return {};

  return createPageMetadata({
    title: procedureData.metaTitle,
    description: procedureData.metaDescription,
    path: "/cirugia-puerto-unico",
    locale: locale as Locale,
    alternateLocalePath: "/cirugia-puerto-unico",
    ogImage: "/images/og-minima-invasion.png",
  });
}

export default async function CirugiaPuertoUnicoPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbJsonLd = generateBreadcrumbJsonLd([
    ...
    { name: procedureData.title, url: `${DOCTOR_INFO.siteUrl}/${locale}/cirugia-puerto-unico` },
  ]);
  ...
  return (
    <>
      ...
      <MedicalDisclaimer />
      <RelatedProcedures currentHref="/cirugia-puerto-unico" />
      <CTASection
        heading={t("cirugiaPuertoUnico.ctaHeading")}
        description={t("cirugiaPuertoUnico.ctaDescription")}
        variant="teal"
      />

...
export async function generateMetadata({ params }: PageProps) {
  ...
  const procedureData = getProcedureBySlug("cirugia-hernias-ventrales", locale as Locale);
  if (!procedureData) return {};

  return createPageMetadata({
    title: procedureData.metaTitle,
    description: procedureData.metaDescription,
    path: "/cirugia-hernias-ventrales",
    locale: locale as Locale,
    alternateLocalePath: "/cirugia-hernias-ventrales",
    ogImage: "/images/og-minima-invasion.png",
  });
}

export default async function CirugiaHerniasVentralesPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbJsonLd = generateBreadcrumbJsonLd([
    ...
    { name: procedureData.title, url: `${DOCTOR_INFO.siteUrl}/${locale}/cirugia-hernias-ventrales` },
  ]);
  ...
  return (
    <>
      ...
      <MedicalDisclaimer />
      <RelatedProcedures currentHref="/cirugia-hernias-ventrales" />
      <CTASection
        heading={t("cirugiaHerniasVentrales.ctaHeading")}
        description={t("cirugiaHerniasVentrales.ctaDescription")}
        variant="teal"
      />

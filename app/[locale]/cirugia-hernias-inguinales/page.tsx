...
export async function generateMetadata({ params }: PageProps) {
  ...
  const procedureData = getProcedureBySlug("cirugia-hernias-inguinales", locale as Locale);
  if (!procedureData) return {};

  return createPageMetadata({
    title: procedureData.metaTitle,
    description: procedureData.metaDescription,
    path: "/cirugia-hernias-inguinales",
    locale: locale as Locale,
    alternateLocalePath: "/cirugia-hernias-inguinales",
    ogImage: "/images/og-minima-invasion.png",
  });
}

export default async function CirugiaHerniasInguinalesPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbJsonLd = generateBreadcrumbJsonLd([
    ...
    { name: procedureData.title, url: `${DOCTOR_INFO.siteUrl}/${locale}/cirugia-hernias-inguinales` },
  ]);
  ...
  return (
    <>
      ...
      <MedicalDisclaimer />
      <RelatedProcedures currentHref="/cirugia-hernias-inguinales" />
      <CTASection
        heading={t("cirugiaHerniasInguinales.ctaHeading")}
        description={t("cirugiaHerniasInguinales.ctaDescription")}
        variant="teal"
      />

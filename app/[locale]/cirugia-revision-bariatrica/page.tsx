...
export async function generateMetadata({ params }: PageProps) {
  ...
  const procedureData = getProcedureBySlug("cirugia-revision-bariatrica", locale as Locale);
  if (!procedureData) return {};

  return createPageMetadata({
    title: procedureData.metaTitle,
    description: procedureData.metaDescription,
    path: "/cirugia-revision-bariatrica",
    locale: locale as Locale,
    alternateLocalePath: "/cirugia-revision-bariatrica",
    ogImage: "/images/og-bariatrica.png",
  });
}

export default async function CirugiaRevisionBariatricaPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbJsonLd = generateBreadcrumbJsonLd([
    ...
    { name: procedureData.title, url: `${DOCTOR_INFO.siteUrl}/${locale}/cirugia-revision-bariatrica` },
  ]);
  ...
  return (
    <>
      ...
      <MedicalDisclaimer />
      <RelatedProcedures currentHref="/cirugia-revision-bariatrica" />
      <CTASection
        heading={t("cirugiaRevisionBariatrica.ctaHeading")}
        description={t("cirugiaRevisionBariatrica.ctaDescription")}
        variant="teal"
      />
